...
extern "C" int DnnlMaxPool(const DLArrayHandle input,const int kernel_H, const int kernel_W,DLArrayHandle output,const int padding, const int stride){
    dnnl_stream_init();
   ...
   // engine eng(engine::kind::cpu, 0); // We create a global engine for simplicity
    ...
    pool_p.execute(engine_stream,
    		{ { DNNL_ARG_SRC, input_data},
		{ DNNL_ARG_DST, output_data } });
...
extern "C" int DnnlMaxPool_Gradient(const DLArrayHandle input,const DLArrayHandle input_grad,const int kernel_H, const int kernel_W,DLArrayHandle output_grad,const int padding, const int stride){
    ...
    auto input_md = memory::desc({ input_N,input_C,input_H,input_W }, memory::data_type::f32, memory::format_tag::nchw);
	auto input_grad_md = memory::desc({ input_N,input_C, pooled_H,pooled_W}, memory::data_type::f32, memory::format_tag::nchw);
	auto output_grad_md = memory::desc({ input_N,input_C,input_H,input_W}, memory::data_type::f32, memory::format_tag::nchw);

    auto input_mem=memory(input_md,eng,(void *)input->data);
    auto input_grad_mem=memory(input_grad_md,eng,(void *)input_grad->data);
    auto output_grad_mem=memory(output_grad_md,eng,(void *)output_grad->data);


	auto pooling_backward_d = pooling_backward::desc(algorithm::pooling_max,
	    output_grad_md, input_grad_md,
		{stride,stride}, {kernel_H,kernel_W}, {padding,padding}, {padding,padding});

	//forward
	auto input_forward_md = memory::desc({ input_N,input_C, pooled_H,pooled_W}, memory::data_type::f32, memory::format_tag::nchw);
	auto input_forward_mem=memory(input_forward_md,eng);
	auto pool_desc = pooling_forward::desc(prop_kind::forward_training,
		algorithm::pooling_max, input_md, input_forward_md,
		{stride,stride}, {kernel_H,kernel_W}, {padding,padding}, {padding,padding});
	auto pool_pd = pooling_forward::primitive_desc(pool_desc, eng);
	...
	auto pooling_backward_pd = pooling_backward::primitive_desc(pooling_backward_d, eng,pool_pd);
