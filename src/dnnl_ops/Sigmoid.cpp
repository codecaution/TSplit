...
extern "C" int DnnlSigmoid(const DLArrayHandle input, DLArrayHandle output)
{
   // engine eng(engine::kind::cpu, 0);
    ...
    auto Sigmoid_d = eltwise_forward::desc(prop_kind::forward_training, algorithm::eltwise_logistic, mat_md, 0.f, 0.f);
    auto Sigmoid_pd = eltwise_forward::primitive_desc(Sigmoid_d, eng);
    auto Sigmoid = eltwise_forward(Sigmoid_pd);

    Sigmoid.execute(engine_stream, {{DNNL_ARG_SRC, src_mem}, {DNNL_ARG_DST, dst_mem}});
    engine_stream.wait();
    return 0;
}
