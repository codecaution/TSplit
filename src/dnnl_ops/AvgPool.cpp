...
extern "C" int DnnlAvgPool(const DLArrayHandle input,
...
//	stream engine_stream(eng);
	...
	auto pool_src_md = memory::desc({ pool_src_tz }, memory::data_type::f32, memory::format_tag::nchw);
	//dst memory
	memory::dims pool_dst_tz = { input_N, input_C, pooled_H, pooled_W };

	auto pool_dst_md = memory::desc({ pool_dst_tz }, memory::data_type::f32, memory::format_tag::nchw);
	...
	auto pool_desc = pooling_forward::desc(prop_kind::forward_inference,
		algorithm::pooling_avg_include_padding, pool_src_md, pool_dst_md,
...
extern "C" int DnnlAvgPool_Gradient(const DLArrayHandle gradient_Y,	const int kernel_H, const int kernel_W,	DLArrayHandle gradient_X, const int padding, const int stride)
{
    ...
    auto gradient_Y_md=memory::desc({ N,C,H,W }, memory::data_type::f32, memory::format_tag::nchw);
    auto gradient_X_md=memory::desc({ N,C,pooled_H,pooled_W }, memory::data_type::f32, memory::format_tag::nchw);
	...
	auto pooling_backward_d = pooling_backward::desc(algorithm::pooling_avg_include_padding,
	    gradient_X_md, gradient_Y_md,
