...
extern "C" int DnnlTanh(const DLArrayHandle input, DLArrayHandle output)
{
    ...
    auto Tanh_d = eltwise_forward::desc(prop_kind::forward_training, algorithm::eltwise_tanh, mat_md, 0.f, 0.f);
    auto Tanh_pd = eltwise_forward::primitive_desc(Tanh_d, eng);
    auto Tanh = eltwise_forward(Tanh_pd);

    Tanh.execute(engine_stream, {{DNNL_ARG_SRC, src_mem}, {DNNL_ARG_DST, dst_mem}});
    engine_stream.wait();
    return 0;
}
