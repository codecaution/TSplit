...
extern "C" int DnnlRelu(const DLArrayHandle input, DLArrayHandle output)
{
    ...
    auto Relu_d = eltwise_forward::desc(prop_kind::forward_training, algorithm::eltwise_relu, mat_md, 0.f, 0.f);
...
extern "C" int DnnlRelu_Gradient(const DLArrayHandle input, const DLArrayHandle in_grad, DLArrayHandle output)
{
    dnnl_stream_init();
    ...
    //forward
    auto Relu_d = eltwise_forward::desc(prop_kind::forward_training, algorithm::eltwise_relu, input_md);
    auto Relu_pd = eltwise_forward::primitive_desc(Relu_d, eng);
    //forward

    auto Relu_gradient_d = eltwise_backward::desc(algorithm::eltwise_relu, in_grad_md,output_md);
    auto Relu_gradient_pd = eltwise_backward::primitive_desc(Relu_gradient_d, eng,Relu_pd);
    auto Relu_gradient_p = eltwise_backward(Relu_gradient_pd);
    Relu_gradient_p.execute(engine_stream,
