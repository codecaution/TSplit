...
extern "C" int DnnlConcat(const DLArrayHandle input_x,const DLArrayHandle input_y, DLArrayHandle output,int axis=0)
{
    dnnl_stream_init();
    ...
    format1[(input_x -> ndim) - 1] = format2[(input_y -> ndim) -1] = format3[(output -> ndim) -1] = 1;
    ...
    concat_e.execute(engine_stream,
...
extern "C" int cpu_Concat_Gradient(const DLArrayHandle output_gradient, DLArrayHandle input_gradient, int axis = 0, int id = 0)
{
    ...
    if(id == 0){
        ...
        for(int i = 0; i < output_gradient -> ndim; i++)
            if(i != axis) end_position *= output_gradient -> shape[i];
            else end_position *= input_gradient -> shape[i];
    }
    else{
        start_position = 1;
        end_position = 1;
        for(int i = 0; i < output_gradient -> ndim; i++){
            ...
            if(i != axis) start_position *= output_gradient -> shape[i];
            else start_position *= (output_gradient -> shape[i]) - (input_gradient -> shape[i]);
        }
    }
