...
__global__ void max_pool_forward_kernel(
  ...
  int DLGpuMax_Pooling2d(const DLArrayHandle input, 
            ...
            const int padding, const int stride, DLStreamHandle stream_handle = NULL, ProfilerHandle p = NULL){
  ...
  if(p != NULL){
    ...
    for(int i = 0; i < input -> ndim; i++)
        size_a *= input -> shape[i];
    for(int i = 0; i < input_grad -> ndim; i++)
        size_b *= input_grad -> shape[i];
    for(int i = 0; i < output_grad -> ndim; i++)
        size_c *= output_grad -> shape[i];
