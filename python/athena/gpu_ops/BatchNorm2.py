...
class Batch_NormalizationOp(Op):
    def __call__(self, node_in, bn_scale, bn_bias, momentum=0.99, eps=0.01):
        ...
        self.eps = eps
        new_node.save_mean = None
        new_node.save_var = None
    ...
    def profile(self, node, input_vals, output_val, is_static = True):
        ...
        if is_static:
            ...
            if node.save_mean == None:
                dev_id = input_vals[0].handle.contents.ctx.device_id
                C = input_vals[0].shape[1]
                node.save_mean = ndarray.array(np.zeros([1, C, 1, 1]), ctx=ndarray.gpu(dev_id))
                node.save_var = ndarray.array(np.zeros([1, C, 1, 1]), ctx=ndarray.gpu(dev_id))

            CuDNN_Batch_Normalization(
                input_vals[0], input_vals[1], input_vals[2], output_val,
                node.save_mean, node.save_var, self.momentum,
    ...
    def compute(self, node, input_vals, output_val, use_numpy=True, stream_handle=None):
        if use_numpy:
            ...
            #     node.save_var = ndarray.array(np.zeros([1, C, 1, 1]), ctx=ndarray.gpu(dev_id))
            # else:
            #     node.save_mean.malloc_itself()
            #     node.save_var.malloc_itself()
            #     node.save_mean.in_gpu = True
            #     node.save_var.in_gpu = True
...
class Batch_Normalization_GradientOp(Op):
    def __call__(self, out_gradient, in_node, bn_scale, forward_node, eps):
        new_node = Op.__call__(self)
        new_node.inputs = [out_gradient, in_node, bn_scale]
        new_node.tmp_gradient_in_arr = None
        new_node.tmp_gradient_bn_bias = None
        new_node.tmp_gradient_bn_scale = None
    ...
    def profile(self, node, input_vals, output_val, is_static = True):
        ...
        if is_static:
            ...
            if node.tmp_gradient_bn_bias == None:
                node.tmp_gradient_bn_bias = ndarray.empty(
                    shape=shapebn, ctx=input_vals[0].ctx)
            else:
                node.tmp_gradient_bn_bias.malloc_itself()
                    
            if node.tmp_gradient_bn_scale == None:
                node.tmp_gradient_bn_scale = ndarray.empty(
                    shape=shapebn, ctx=input_vals[0].ctx)
            else:
                node.tmp_gradient_bn_scale.malloc_itself()

            if node.tmp_gradient_in_arr == None:
                node.tmp_gradient_in_arr = ndarray.empty(
                    shape=input_vals[1].shape, ctx=input_vals[0].ctx)
            else:
                node.tmp_gradient_in_arr.malloc_itself()
                    
            CuDNN_Batch_Normalization_gradient(input_vals[0], input_vals[1], input_vals[2],
                                               node.tmp_gradient_in_arr, node.tmp_gradient_bn_scale,
                                               node.tmp_gradient_bn_bias, self.forward_node.save_mean,
                                               self.forward_node.save_var, self.eps, None, node.profiler)
    ...
    def compute(self, node, input_vals, output_val, use_numpy=True, stream_handle=None):
        ...
        if use_numpy:
            ...
            CuDNN_Batch_Normalization_gradient(input_vals[0], input_vals[1], input_vals[2],
                                               node.tmp_gradient_in_arr, node.tmp_gradient_bn_scale,
                                               node.tmp_gradient_bn_bias, self.forward_node.save_mean,
                                               self.forward_node.save_var, self.eps, stream_handle, None)
...
def batch_normalization_gradient_op(out_gradient, in_node, bn_scale, forward_node, eps):
    ...
    """
    return Batch_Normalization_GradientOp()(out_gradient, in_node, bn_scale, forward_node, eps)
