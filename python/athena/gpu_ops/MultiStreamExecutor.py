...
class MultiStreamExecutor(object):
    ...
    def memory_plan(self, feed_shapes):
        ...
        for node, shape in self.node_to_shape_map.items():
            ...
            if isinstance(node.op, PlaceholderOp) and node.tensor_value is not None:
                ...
                if (self.policy == 'swap' and node.swap) or self.policy == 'vdnn':
                    if isinstance(node, ndarray.NDArray) and ndarray.is_gpu_ctx(node.ctx):
                        arr = arr.asnumpy()
