...
def init():
    ...
    matmulop = ad.matmul_op(X, W0)
    
    rand = np.random.RandomState(seed=123)
    X_val = np.random.normal(scale=0.1, size=(N, H))
    W0_val = np.random.normal(scale=0.1, size=(H, W))

    X_val = ndarray.array(X_val, ctx = ndarray.gpu(0))
    W0_val = ndarray.array(W0_val, ctx = ndarray.gpu(0))
    output_val = ndarray.empty((N, W), ctx = ndarray.gpu(0))
    
    matmulop.op.profile(matmulop, [X_val, W0_val], output_val, is_static = False)
    
     
def test_batch_size():
    print("test batch size")
    output_file = open("./matmul_exp/matmulForward_batch_size_2.txt", 'w', buffering=1)
    output_file.write("Input size = batch_size, 1024\n")
    output_file.write("Param size = 1024, 1024\n")
    output_file.write("Output size = batch_size, 1024\n")

    for batch_size in range(1, 1500):
        N, H, W = batch_size, 1024, 1024
        W0 = ad.Variable(name = "W0")
        X = ad.Variable(name = "X")
        Y = ad.Variable(name = "Y")
