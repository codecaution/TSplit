...
static const char *DataTypeName[] = {"CHAR",
                                     "INT8",
                                     "INT16",
                                     "INT32",
                                     "INT64",
 ...
 */
template <typename V, typename W> inline bool SameType() {
  return std::is_same<typename std::remove_cv<V>::type, W>::value;
}
/**
 * \brief return the DataType of V
 */
template <typename V> DataType GetDataType() {
...
struct Meta {
  ...
  Meta()
      : head(kEmpty), app_id(kEmpty), customer_id(kEmpty), timestamp(kEmpty),
        ...
        pull(false), simple_app(false), priority(kEmpty),
...
struct Message {
   ...
   */
  template <typename V> void AddData(const SArray<V> &val) {
